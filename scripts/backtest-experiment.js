#!/usr/bin/env node
/**
 * Experimental backtest for parameter testing with current system parameters
 */
...
// Updated CONFIG to match current system
const CONFIG = {
  minEntrySpread: parseFloat(process.argv[2]) || 4.0,   // Current system: 4.0%
  targetSpread: parseFloat(process.argv[3]) || 2.5,      // Current system: 2.0-3.0%, test 2.5%
  maxHoldMinutes: parseFloat(process.argv[4]) || 60,     // Current system: 60min, test variations
  minHoldMs: 5 * 60 * 1000,                             // Current: 5 min
  positionSizeUsd: 10,
...
CONFIG.maxHoldMs = CONFIG.maxHoldMinutes * 60 * 1000;

async function run() {
  console.log(`\nBacktest: ${CONFIG.minEntrySpread}% entry, ${CONFIG.targetSpread}% exit, ${CONFIG.maxHoldMinutes}min hold, 3 days`);
  ...
  console.log(`Entry: ${CONFIG.minEntrySpread}% | Exit: ${CONFIG.targetSpread}% | Hold: ${CONFIG.maxHoldMinutes}min`);
  console.log('='.repeat(50));
