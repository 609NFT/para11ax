...
| **Feb 8** | **🔥 4.25%** | 48 | **43.8%** | **+$6.75** | **+45%** | 🚀 **SIGNIFICANT IMPROVEMENT: 4.25% entry threshold vs current baseline (+$4.66 PnL) shows substantial PnL increase (+45% improvement). Higher win rate (33.3%→43.8% = +31% relative). Fills optimal gap between tested 4.2%(-1%) and variable 4.5% results. Pattern: 4.0% < 4.05%(+15%) < 4.1%(-4%) < 4.2%(-1%) < 4.25%(+45%) suggests sweet spot around 4.25%** |
...
- **Longer**: 90min (-36% PnL, 89% max_hold exits), spreads widen back during extended holds. 75min shows no improvement (0% change).
